              ...
              <b-row>
                ...
                <b-col md="12" class="mb-3">
                  <b-form-group label="Transfer Type *">
                    ...
                    <small class="text-muted">Destination requests require a note and source-warehouse approval.</small>
                  </b-form-group>
                </b-col>
                ...
                <b-col v-if="transfer.transfer_type === 'direct_transfer'" lg="4" md="4" sm="12" class="mb-3">
                  <b-form-group label="Driver">
                    <v-select v-model="transfer.driver_id" :reduce="option => option.value" placeholder="Choose active driver" :options="drivers.map(driver => ({ label: driver.name + (driver.phone ? ' - ' + driver.phone : ''), value: driver.id }))" />
                  </b-form-group>
                </b-col>
                <b-col v-if="transfer.transfer_type === 'direct_transfer'" lg="4" md="4" sm="12" class="mb-3">
                  <b-form-group label="Vehicle details (optional)"><b-form-input v-model.trim="transfer.vehicle_details" maxlength="191"></b-form-input></b-form-group>
                </b-col>
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <b-form-group label="Required Date (optional)"><b-form-input type="date" v-model="transfer.required_date"></b-form-input></b-form-group>
                </b-col>
                <!-- From warehouse -->
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="From Warehouse" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('FromWarehouse') + ' ' + '*'">
                      <v-select
                        ...
                        :disabled="details.length > 0 || (source_locked && transfer.transfer_type === 'direct_transfer')"
                  ...
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr v-if="detail.is_imei && detail.del !== 1 && detail.no_unit !== 0" :key="'serial-' + detail.detail_id">
                ...
                <b-col md="12">
                  <validation-provider name="Transfer Note" :rules="{ required: transfer.transfer_type === 'destination_request', max: 5000 }" v-slot="validationContext">
                    <b-form-group :label="transfer.transfer_type === 'destination_request' ? 'Stock Request Note *' : 'Transfer Note (optional)'">
                      <b-form-textarea v-model.trim="transfer.notes" rows="4" maxlength="5000" :state="getValidationState(validationContext)" :placeholder="transfer.transfer_type === 'destination_request' ? 'Explain why this stock is required' : 'Optional transfer details'"></b-form-textarea>
                ...
                <b-col md="12">
                  <b-form-group>
                    <b-button variant="outline-primary" class="mr-2" @click="Submit_Transfer('draft')" :disabled="SubmitProcessing || hasBatchValidationErrors">
                      ...
                      {{ SubmitProcessing ? ($t('Saving') || 'Saving...') : 'Save Draft' }}
                    </b-button>
                    <b-button v-if="transfer.transfer_type === 'direct_transfer' && currentUserPermissions && currentUserPermissions.includes('transfer_dispatch')" variant="primary" class="mr-2" @click="Submit_Transfer('dispatch')" :disabled="SubmitProcessing || hasBatchValidationErrors"><lucide-icon name="truck" class="mr-1" />Dispatch Transfer</b-button>
                    <b-button v-if="transfer.transfer_type === 'destination_request'" variant="primary" @click="Submit_Transfer('request')" :disabled="SubmitProcessing || hasBatchValidationErrors"><lucide-icon name="send" class="mr-1" />Submit Stock Request</b-button>
...
export default {
  ...
  computed: {
    ...
    destinationWarehouseOptions() {
      if (this.transfer.transfer_type === 'destination_request' && this.assigned_warehouse_id) return this.warehouses || [];
      return (this.to_warehouses || []).filter(warehouse => Number(warehouse.id) !== Number(this.transfer.from_warehouse));
    },

    hasBatchValidationErrors() {
      if (!Array.isArray(this.details)) return false;
      for (const d of this.details) {
        if (!d || !d.is_batch_tracked || d.del === 1 || d.no_unit === 0) continue;
    ...
    verifiedForm() {
      if (this.details.length <= 0) {
        ...
        for (var i = 0; i < this.details.length; i++) {
          ...
          if (this.details[i].is_imei && this.details[i].del !== 1 && this.details[i].no_unit !== 0) {
    ...
    GetElements() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.Calcul_Total();
          // Pharmacy: hydrate available batches for preloaded batch-tracked lines so the
          // picker dropdown is populated when the user opens the edit page.
          if (Array.isArray(this.details)) {
            for (const d of this.details) {
              if (d && d.is_batch_tracked) {
                this.fetch_batches_for_detail(d);
