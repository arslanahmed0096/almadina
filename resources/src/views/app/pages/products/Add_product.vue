    ...
    onScan (decodedText, decodedResult) {
     ...
     generateNumber() {
       ...
       const firstCategoryId =
         Array.isArray(this.product.assigned_category_ids) && this.product.assigned_category_ids.length
           ? this.product.assigned_category_ids[0]
           : "";
       axios
         .get("products-next-code", {
           params: {
             category_id: firstCategoryId || undefined
           }
         })
         .then(({ data }) => {
           this.productPrefix = data.product_prefix || this.productPrefix || "AME";
           this.product.code = data.code || this.product.code;
         })
         .catch(() => {
           this.makeToast("danger", "Unable to generate product code.", this.$t("Failed"));
         });
      },
    ...
    GetElements() {
      axios
        ...
        .then(response => {
          ...
          this.productPrefix = response.data.product_prefix || "AME";
          const byWh = {};
          ...
          if (!(this.$route && this.$route.query && this.$route.query.duplicate) && !this.product.code) {
            this.product.code = response.data.next_product_code || this.product.code;
          }
    ...
    if (duplicateId) {
      ...
      axios
        ...
        .then((response) => {
          ...
          this.product.company_rb_price = p.company_rb_price || "";
          this.product.mrp_price = p.mrp_price || "";
          this.product.price = p.price || "";
          this.product.fix_price = p.fix_price || p.price || "";
          ...
          if (Array.isArray(p.ProductVariant) && p.ProductVariant.length) {
            this.variants = p.ProductVariant.map((v, idx) => ({
              ...
              code: v.code,
              cost: v.cost,
              company_rb_price: v.company_rb_price != null ? v.company_rb_price : '',
              mrp_price: v.mrp_price != null ? v.mrp_price : '',
              price: v.price,
              fix_price: v.fix_price != null ? v.fix_price : v.price,
