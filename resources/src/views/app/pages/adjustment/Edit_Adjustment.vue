                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        >
                          ...
                          <td v-if="detail.product_type != 'is_combo'">
                            <select
                              v-model="detail.type"
                          ...
                          <td v-if="detail.product_type == 'is_combo'">
                            <span v-if="detail.type == 'add'">{{$t('Addition')}}</span>
                            <span v-if="detail.type == 'sub'">{{$t('Subtraction')}}</span>
                          </td>
                          <td>
                            <a
...
export default {
  ...
  computed: {
    ...
    // allocation blocks the submit button. Excludes soft-deleted rows (del === 1).
  ...
  methods: {
    ...
    fetch_batches_for_detail(detail) {
      ...
      const variantSeg = (detail.product_variant_id != null && detail.product_variant_id !== "")
        ...
        : 0;
      // Capture pre-existing batches loaded from the server so we can merge their qty
      // back into qty_available (the API returns post-consumption availability).
      const existingQtyById = {};
      const existingBatches = Array.isArray(detail.batches) ? detail.batches : [];
