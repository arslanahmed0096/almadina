                        ...
                        <tr v-if="detail.is_batch_tracked" :key="'b-' + detail.detail_id" style="background: transparent;">
                          <td colspan="7" style="padding: 0; border-top: 0;">
                            <div style="margin: 6px 8px 14px 8px; border: 1px solid #e0e7ff; border-radius: 10px; overflow: visible; background: #f8faff;">
                              ...
                              <div v-else-if="detail.type !== 'add' && !(detail.available_batches && detail.available_batches.length)" style="padding: 12px 14px; text-align: center; color: #b91c1c; font-size: 12px; background: #fef2f2;">
                              ...
                              <div v-else-if="!detail.batches || detail.batches.length === 0" style="padding: 12px 14px; text-align: center; color: #6b7280; font-size: 12px;">
                                <lucide-icon name="info" style="margin-right: 6px;" />
                                <span v-if="detail.type === 'add'">
                                  {{ $t('Adjustment_Add_Batch_Hint') || 'For "Addition" adjustments, the per-batch ledger is only updated when you provide explicit batch info.' }}
                                </span>
                                <span v-else>
                                  {{ $t('Adjustment_Sub_Batch_Hint') || 'Click "Add" to pick a batch (or leave empty to auto-allocate FEFO on save).' }}
                                </span>
                              </div>
                              ...
                              <table v-else style="width: 100%; border-collapse: collapse; font-size: 12px;">
                                ...
                                <tbody>
                                  <tr v-for="(b, bIdx) in detail.batches" :key="'bb-' + detail.detail_id + '-' + bIdx" :style="{ background: bIdx % 2 === 1 ? '#f8faff' : '#ffffff', borderTop: '1px solid #e0e7ff' }">
                                    ...
                                    <td style="padding: 6px 8px; vertical-align: middle;">
                                      <b-form-input
                                        ...
                                        :style="{
                                          ...
                                          height: '30px',
                                          borderRadius: '6px',
                                          borderColor: (detail.type !== 'add' && Number(b.qty) > (Number(b.qty_available) || 0)) ? '#fca5a5' : undefined
                                        }"
                                      ></b-form-input>
                                    </td>
...
export default {
  ...
  computed: {
    // Mirror of create_sale.vue: any batch-tracked line without a complete, valid batch
    // allocation blocks the submit button. Re-evaluates reactively as the user types.
    hasBatchValidationErrors() {
      ...
      for (const d of this.details) {
        ...
        for (const b of batches) {
          ...
          if (!(q > 0)) return true;
          // qty_available only matters when subtracting — adds don't consume.
        ...
        // Normalize via 4 decimal rounding to absorb input/float drift.
        const total = Math.round(batches.reduce((sum, b) => sum + (Number(b.qty) || 0), 0) * 10000) / 10000;
    ...
    firstBatchErrorMessage() {
      ...
      for (const d of this.details) {
        ...
        for (const b of batches) {
          ...
          if (d.type !== "add" && q > (Number(b.qty_available) || 0) + 0.01) {
  ...
  watch: {
    // Auto-sync the single-batch case: when the user changes the line qty and there's
    // exactly one batch row, mirror the new qty into the batch so the user doesn't
    // have to update both fields.
    "details": {
      deep: true,
      handler(details) {
   ...
   //------------------------------------ Get Products By Warehouse -------------------------\\
    ...
    has_batch_validation_errors() {
      ...
      for (const d of this.details) {
        if (!d || !d.is_batch_tracked) continue;
        const batches = Array.isArray(d.batches) ? d.batches : [];
        // Empty batches OK — backend skips ('add') or auto-FEFOs ('sub').
        if (batches.length === 0) continue;
        for (const b of batches) {
          if (!b.product_batch_id) return true;
          if (!Number.isFinite(Number(b.qty)) || Number(b.qty) <= 0) return true;
          // Over-allocation only matters for "sub" — "add" credits batches without consuming.
          if (d.type !== 'add' && Number(b.qty) > (Number(b.qty_available) || 0) + 0.0001) return true;
        }
    ...
    verifiedForm() {
      if (this.details.length <= 0) {
        ...
        if (this.has_batch_validation_errors()) {
          this.makeToast(
            "danger",
    ...
    batchBadgeTitle(detail) {
      if (!detail) return "";
      if (detail.type === "add") {
        return this.$t("Adjustment_Add_Batch_Hint") || 'For "Addition" adjustments, the per-batch ledger is only updated when you provide explicit batch info.';
      }
      return this.$t("Adjustment_FEFO_Hint") || "Oldest-expiring batches will be auto-allocated (FEFO) when this adjustment is saved.";
    },
