...
class ProductsController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \DB::transaction(function () use ($request) {
                ...
                $Product->preorder_note = $Product->is_preorder && $request->filled('preorder_note') && $request['preorder_note'] !== 'null'
                    ? $request['preorder_note'] : null;
    ...
    public function create(Request $request)
    {
        ...
        return response()->json([
            ...
            'product_prefix' => $this->getProductPrefix(),
            'next_product_code' => $this->generateNextProductCode(),
        ]);

    }

    public function nextProductCode(Request $request)
    {
        $this->authorizeForUser($request->user('api'), 'create', Product::class);
        ...
        return response()->json([
            'product_prefix' => $this->getProductPrefix(),
            'code' => $this->generateNextProductCode($categoryId),
        ]);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $item['is_preorder'] = (bool) $Product->is_preorder;
        $item['preorder_available_date'] = $Product->preorder_available_date
            ? $Product->preorder_available_date->format('Y-m-d') : '';
        $item['preorder_limit'] = $Product->preorder_limit !== null ? $Product->preorder_limit : '';
        $item['preorder_note'] = $Product->preorder_note ?? '';
     ...
     * name, retail_price/price (opt), cost (opt), category, brand (opt), stock_alert (opt), note (opt)
     */
            ...
            DB::transaction(function () use ($toCreate, $warehouses) {
                ...
                foreach ($toCreate as $data) {
                    $product = Product::create([
                        ...
                        'discount' => 0,
                        'discount_method' => 1,
                        'code' => $this->generateNextProductCode((int) $data['category_id']),
                        // payload
                        ...$data,
                    ]);
     ...
     * Expected headers: name, retail_price/price (opt), category, wholesale_price (opt), min_price (opt), brand (opt), note (opt)
     */
    ...
    public function import_variant_products(Request $request)
    {
        ...
        foreach ($rows as $raw) {
            ...
            if ($parentName === '' || trim((string) $brandName) === '') {
                continue;
            }
    ...
    protected function getProductPrefix(): string
    {
        $setting = Setting::query()->first();
        $prefix = trim((string) optional($setting)->product_prefix);

        return $prefix !== '' ? strtoupper($prefix) : 'AME';
    }

    protected function generateNextProductCode(?int $categoryId = null): string
    {
        $prefix = $this->getProductPrefix();
        $categoryCode = $this->getProductCategorySegment($categoryId);
        $year = now()->format('y');
        $base = $prefix.'-'.$categoryCode.'-'.$year;
        $pattern = '/^'.preg_quote($base, '/').'(\d+)$/i';
        $max = 0;
        ...
        Product::query()
            ...
            ->pluck('code')
            ->each(function ($code) use ($pattern, &$max) {
    ...
    protected function getProductCategorySegment(?int $categoryId = null): string
    {
        if (! $categoryId) {
            return 'GEN';
        }
        ...
        $lettersOnly = preg_replace('/[^A-Za-z]/', '', strtoupper((string) $category->name));
        $segment = substr($lettersOnly ?: 'GEN', 0, 3);

        return str_pad($segment, 3, 'X');
    }

    protected function firstOrCreateImportCategory(string $name): Category
    {
        $name = trim($name);
        ...
        if ($existing) {
            ...
            if (! $existing->code) {
                $existing->code = $this->generateUniqueImportCategoryCode($name, $existing->id);
            }
            if (property_exists($existing, 'status') && $existing->status === null) {
                $existing->status = 1;
            }
            $existing->save();
        ...
        return Category::create([
            'name' => $name,
            'code' => $this->generateUniqueImportCategoryCode($name),
            'status' => 1,
        ]);
    }

    protected function generateUniqueImportCategoryCode(string $name, ?int $ignoreId = null): string
    {
        ...
        while ($this->importCategoryCodeExists($candidate, $ignoreId)) {
    ...
    protected function importCategoryCodeExists(string $code, ?int $ignoreId = null): bool
    {
