...
function currencySymbol() {
  var m = document.querySelector('meta[name="currency"]');
  return m && m.content ? m.content : '$';
}
function fmtMoney(v, sym) {
  var s = sym || currencySymbol();
  return s + Number(v || 0).toLocaleString('en-US', {
...
(function initCart() {
  ...
  function calc(c) {
    c.subtotal = c.items.reduce(function (a, i) {
  ...
  function idx(c, id) {
    return c.items.findIndex(function (i) {
      return String(i.id) === String(id);
    });
  }
  window.CartLS = {
    get: load,
    add: function add(item, qty) {
      qty = qty == null ? 1 : qty;
      var allowOverselling = window.__ALLOW_OVERSELLING__ !== false;
      var c = load();
      var i = idx(c, item.id);
      var stock = item && (typeof item.stock === 'number' || typeof item.stock !== 'undefined' && item.stock !== null) ? Number(item.stock) : null;
      var requested = Number(qty) || 1;
      if (i > -1) {
        var existingQty = c.items[i].qty || 0;
        var newQty = existingQty + requested;
        if (!allowOverselling && stock != null) {
          var maxQty = Math.max(0, stock);
        ...
        var addQty = Math.max(1, requested);
        if (!allowOverselling && stock != null) {
          var maxStock = Math.max(1, stock);
    ...
    setQty: function setQty(id, q) {
      var c = load();
      var i = idx(c, id);
      if (i < 0) return c;
      var allowOverselling = window.__ALLOW_OVERSELLING__ !== false;
      var requested = Math.max(1, Number(q) || 1);
      if (!allowOverselling && c.items[i].stock != null) {
        var maxStock = Math.max(0, Number(c.items[i].stock));
...
window.showStockAlert = function (msg) {
  var el = document.getElementById('store-stock-toast');
  if (!el) return;
  el.textContent = msg;
  el.classList.remove('hidden');
  clearTimeout(window.__stockToastT);
  window.__stockToastT = setTimeout(function () {
...
function updateCartBadge() {
  var c = window.CartLS.get();
  var count = c.items.reduce(function (a, i) {
    return a + (i.qty || 0);
  }, 0);
...
if (!window.__CART_WIREUP__) {
  ...
  document.addEventListener('click', function (e) {
    ...
    var stockVal = btn.dataset.stock !== undefined && btn.dataset.stock !== '' ? parseInt(btn.dataset.stock, 10) : undefined;
    var item = {
      id: btn.dataset.id,
    ...
    var addedLabel = btn.dataset.addedLabel || window.__MSG_ADDED__ || 'Added';
    btn.disabled = true;
 ...
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('dialog', function () {
  ...
  return {
    ...
    init: function init() {
      ...
      this.$el.addEventListener('ui:open', function () {
        return _this.open();
      });
      this.$el.addEventListener('ui:close', function () {
        return _this.close();
      });
      this.$el.addEventListener('ui:toggle', function () {
    ...
    open: function open() {
      ...
      this.$nextTick(function () {
        var panel = _this2.$refs.panel;
        if (panel) {
          var focusable = panel.querySelector('[autofocus], input, select, textarea, button');
...
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('drawer', function () {
  ...
  return {
    ...
    init: function init() {
      ...
      this.$el.addEventListener('ui:toggle', function () {
        return _this3.isOpen ? _this3.close() : _this3.open();
      });
    },
    open: function open() {
      this.isOpen = true;
 ...
 *         </div>
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('tabs', function (initial) {
  return {
    tab: initial,
    is: function is(name) {
 ...
 *         </div>
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('qtyStepper', function (initial, max) {
  return {
 ...
 *         </div>
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('searchBox', function (endpoint) {
  return {
    q: '',
    results: [],
    loading: false,
    ...
    fetch: function (_fetch) {
      ...
      fetch(this.endpoint + '?q=' + encodeURIComponent(term), {
        ...
        _this4.results = Array.isArray(data) ? data : [];
      })["catch"](function () {
        _this4.results = [];
      })["finally"](function () {
 ...
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('miniCart', function () {
  return {
    items: [],
    subtotal: 0,
    grand: 0,
    ...
    money: function money(v) {
      return this.hidePrices ? '—' : fmtMoney(v, this.currency);
    },
    lineTotal: function lineTotal(it) {
      return this.money((it.price || 0) * (it.qty || 1));
    },
    maxFor: function maxFor(it) {
      if (this.allowOverselling) return null;
      return it.stock != null ? Math.max(1, Number(it.stock)) : null;
    },
    inc: function inc(it) {
      var max = this.maxFor(it);
      var next = (it.qty || 1) + 1;
    ...
    dec: function dec(it) {
      window.CartLS.setQty(it.id, Math.max(1, (it.qty || 1) - 1));
    },
    setQty: function setQty(it, v) {
      window.CartLS.setQty(it.id, Math.max(1, parseInt(v, 10) || 1));
    },
    remove: function remove(it) {
      window.CartLS.remove(it.id);
    },
    clear: function clear() {
      window.CartLS.clear();
    },
    checkout: function checkout(url) {
 ...
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('sidebarMenu', function () {
  return {
    open: null,
    is: function is(id) {
      return this.open === id;
 ...
 */
alpinejs__WEBPACK_IMPORTED_MODULE_0__["default"].data('pageLoader', function () {
  return {
    init: function init() {
      var _this6 = this;
